...
export const getThresholdQueryState = (enabled: boolean, query: ThresholdQueryResult) => ({
    // Previous data can still be drawn, but must never be imported as the
    // result of the newly selected parameters. A paused request is pending too.
    isReady: enabled && query.isSuccess && query.data !== undefined &&
